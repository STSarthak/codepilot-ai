...
public class AuthServiceImpl implements AuthService {
    ...
    public AuthResponse signup(SignupRequest request) {
        ...
        Long planId = planService.getAllActivePlans().stream()
                .filter(planResponse -> planResponse.name().equalsIgnoreCase(FREE_PLAN))
                .map(PlanResponse::id)
                .findFirst()
                .orElseThrow(() -> new BadRequestException("Free Plan not found in the database."));

        subscriptionService.createFreeSubscription(user.getId(), planId);
